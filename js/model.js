...
function stickFloder(StickArr) {
    ...
    var thisVal = getchilds(liArr[liArr.length-1].id);
    // console.log(thisVal,liArr[liArr.length-1])
    if(thisVal.length){
        thisVal.forEach(function (item) {
            if(item.name == Stick.name){
                elements.info.innerHTML = '已经有了，不能再复制了';
